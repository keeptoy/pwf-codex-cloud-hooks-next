...
| Phase 4.5 | F2B autonomous activation Discovery 里程碑：冻结 profile-bound commit point、owned attestation/nonce/ledger admission、Git-backed 首选与 same-chat 备选 Cloud consent 路线、版本失败切换和现有不可达 seam 兜底；结论为 conditional-go，不表示 autonomous 或 Cloud opt-in 已实施 | [`phase-4.5-f2b-autonomous-activation-discovery.md`](phase-4.5-f2b-autonomous-activation-discovery.md) |
