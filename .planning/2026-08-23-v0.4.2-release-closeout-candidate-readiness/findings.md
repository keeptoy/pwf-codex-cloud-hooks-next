...
- 第二轮退役准入审计枚举出24个非活动planning scope：每个都具备task_plan/findings/progress三件套，72个文件全部被Git追踪，目录内零脏改动，current tree没有对这些exact scope路径的外部引用。23个scope无未勾选项；唯一例外`v0.4.1-installer-path-safety`的未勾选项是Linux/POSIX真实平台证据，已由后继`v0.4.1-linux-path-safety-gate`完成并提升到Phase 4.13，原计划自身明确`PLAN_CLOSED`，因此不存在待办丢失。
- v0.4.1 acceptance自己的P9-F已经冻结同一precedent：版本退出accepted工作树席位后，本地guide/bootstrap可以清退，immediate fallback继续由immutable source/tag/Release/provenance与动态publication oracle恢复。维护者现明确授权按同一规则清退v0.4.1本地副本；远端资产保持不可变。
- 当前publication oracle不是硬编码v0.4.1/v0.4.0，而是动态读取ROADMAP accepted/immediate fallback并从provenance重建。角色轮转后无需改写其核心逻辑，但本地尚缺`v0.4.2` tag；必须先核对远端tag精确指向C0并创建同身份本地tag，才能验证v0.4.2 accepted + v0.4.1 fallback。
- v0.4.2的Release事务有两个不同的Git恢复点：正式tag/C0 `d51f291…`保存已通过Source/Candidate的zero-hash外部bootstrap输入和可复现ZIP；publication后的commit `a7d9a54…`保存SHA为`4c04b475…`的sealed公开bootstrap。publication oracle必须分别读取这两个immutable来源，不能要求C0本身包含尚未发生的seal，也不能长期依赖current branch副本。
- 最终C2审计确认：本地/远端v0.4.2 tag都精确指向C0；24个非活动scope与v0.4.1 current guide/bootstrap已经退役；v0.4.2 current guide/bootstrap、活动planning、远端三版本资产及稳定contracts/runtime/templates/history继续保留。ROADMAP角色为v0.4.2 accepted、v0.4.1 immediate fallback、v0.4.0 deeper fallback。
