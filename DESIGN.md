...
| [`f3c-rollback-protocol.test.js`](tests/f3c-rollback-protocol.test.js) | F3C 双身份 rollback evidence、runtime-only revival negative 与自包含 operator guide | accepted/current role、disarm intent、immutable v0.3.5 runtime、Release-excluded helper | repository/Linux no-live；Windows revival case诚实 SKIP，真实 rollback另行验收 |
