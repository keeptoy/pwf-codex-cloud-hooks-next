...
- 维护者明确给出“全部通过”“可直接标记 PASS”等最终验收结论后，智能体应以该结论和已提供证据完成
  直接写回，不得擅自重新分类、要求重跑或扩展验收范围。
- 若智能体发现潜在矛盾，或认为需要增加分析、测试、文档改动或调整结论，应先只提交意见、依据、影响
  范围和建议动作，等待维护者明确授权后再修改文件或创建 commit。
- 维护者已经明确指定的修改属于已授权范围，可以直接执行；不得重复索要确认，也不得借此扩大到未指定
  事项。
- 在用户已授权的本地改动范围内，每个重要且可独立恢复的阶段通过相称验证后，智能体应自动创建本地
  commit，不再为普通本地 commit 单独等待一次确认；提交必须范围单一、信息明确，不得混入用户已有改动。
- 如果工作树包含归属不明或与本任务重叠的用户改动，先保留并分离；无法安全分离时停止并请维护者决定，
...
- 智能体可以在活动任务授权范围内执行远端只读查询，准备精确命令和待推送的本地 commit。GitHub Release Latest
  promotion confirmation的成功条件、正常路径不另设独立postflight及异常诊断边界只读
  [`ROADMAP`](ROADMAP.md#github-release-latest-promotion-confirmation)；维护者明确报告正常成功时直接记录，结果未知、状态矛盾或
  专项authority要求诊断时再做有界只读查询。本文件不复制Release UI判断规则。
  最终交接必须明确本地 commit、待维护者执行的远端动作和核验结果。
- 大白话：本地改完、测完就自动存成 commit；联网改 GitHub 的动作统一由维护者点按钮或执行命令。这样减少
  权限确认、网络重试和来回等待，通常也更省 token，但不得因此跳过测试、hash、资产或远端结果确认；Latest promotion按
  上述ROADMAP链接处理。
