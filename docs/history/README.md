...
冻结；capsule只允许事实纠错与link maintenance，Discovery record还可以按证据追加post-*状态，但不能重写原始决定。

## 收录边界

- `RETROSPECTIVE_CAPSULE`只总结已经关闭并有不可变证据的Product Phase、patch/governance train或回顾性interlude；
...
- v0.4.0首次完整Release discovery现回顾性编号为Phase 4.12；原P9-A～P9-F名称、证据anchors与历史语义继续保留。
  ...
  风险时才增加专项Discovery/Release hardening record。
- 不复制 production source、脚本、fixture、验收全文、SHA 表、测试计数或旧 planning。
- history对象不维护当前candidate、accepted、rollback、PASS/PENDING或下一步状态。
- 每个history对象最多保留一个immutable source snapshot，不直接链接已清退Phase/Round设计文档或验收全文。
...
先用本索引定位history object，并直接阅读对应record。末尾的 source snapshot 只是 cold evidence，不是当前 authority；
需要审计历史来源时才进入该 commit。如果history object与不可变历史字节冲突，通过新的事实纠错提交修复record；
不得把旧设计重新提升为当前规范，也不得改写旧 commit、tag、Release 或 acceptance。

新增history object或在上下文丢失后恢复写作边界时，复制 [`Phase 历史对象模板`](../phase-history-template.md)。模板
只提供写作提示，不是 machine contract；现有history objects不会因为模板文字调整而被批量重写。
