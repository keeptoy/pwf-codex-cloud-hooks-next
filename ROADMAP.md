...
| 当前开发列车 | `v0.4.0-dev` zero-hash development candidate；F0/F1A/F1B complete；F2A/F2B/F3A Source/Candidate Cloud PASS；F3B0 Discovery 与 F3B1 no-live protocol materialization complete；F3B2 smart local validation chain ready、Cloud live pending；F3B3/F3B4/F3C 未授权 |
...
| 当前 programme 边界 | Product Phase 4；F1 foundation、F2A/F2B read-only consumer、F3A lifecycle foundation、F3B0/F3B1 complete；F3B2 exact local refs 已冻结，等待维护者 push 与 Cloud Fresh/UserPrompt/real Resume/disarm/re-arm；尚无 live PASS |
...
隔离 DAG、bounded tamper 与 exact evidence record 物化并完成 repository-only/no-live dry run。维护者随后已授权 F3B2
smart live；markerless runtime source 与隔离 smart validation refs 已在本地冻结，等待维护者远端传输和真实 Cloud
evidence。F3B3、F3B4 与 F3C 仍未授权。如果
...
|---|---|---|---|---|
| 4 | `0.4.0-*` | owned v3 state foundation；显式 smart/autonomous opt-in | F0 → F1A/F1B → F2A/F2B → F3A lifecycle foundation → F3B0～F3B4 Fresh/Resume/disarm/re-arm → F3C rollback；legacy 默认不变 | F1/F2A/F2B/F3A Source/Candidate Cloud PASS；F3B0/F3B1 complete；F3B2 smart local chain ready / Cloud pending；F3B3/F3B4/F3C 未授权 |
...
完成 no-live dry run，production/contract/Release bytes不变。维护者随后单独授权 F3B2；markerless runtime source 和
隔离的 `S_PREP → S_ARM → S_DISARM → S_REARM` local refs 已冻结，等待维护者 push 与真实 Cloud lifecycle。尚未取得 live
evidence，不得写 F3B2 PASS。F3B 后继顺序为：B2 单独验证 smart；B3 再验证
