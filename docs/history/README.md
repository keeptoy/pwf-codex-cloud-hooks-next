...
| Phase 4.10 | F3C disarm-first rollback 小型 Discovery：区分 publication identity 与 installed-state downgrade，冻结 committed disarm → current-owned uninstall → immutable v0.3.5 clean install → current forward recovery、smart/autonomous 独立 live 与 evidence v2 分工；只 conditional-go 到 F3C1 protocol/no-live materialization，不执行 rollback、不创建 refs | [`phase-4.10-f3c-rollback-discovery.md`](phase-4.10-f3c-rollback-discovery.md#phase-4-10-decision) |
