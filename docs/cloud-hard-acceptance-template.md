...
- 普通Release不要求standing Phase 9。Source/Candidate前只做非破坏性candidate admission preflight；两个真实retirement
  checkpoint分别承接Source/Candidate PASS后的第一通道closeout与GitHub Release Latest promotion confirmation后的Release退出治理。它们不创建Cloud
  task或验收轮次。
...
Published Release fresh environment
  ...
  -> discard environment
  -> maintainer GitHub Release Latest promotion confirmation
...
本步骤明确取代该限制。为完成 C，优先使用独立的只读文件工具检查 `.planning/.active_plan`、新生成的
`.planning/PLAN_ID` 目录及下列目标文件。如果当前会话没有独立的只读文件工具，可以仅在 apply_patch 前执行有界的
只读 Shell preflight：Shell 只允许检查这些 exact path 的存在性与类型，并读取 `.planning/.active_plan` 当前内容，
不得读取其他仓库文件。
...
Published Release、GitHub Release Latest promotion confirmation和role-window closeout retirement checkpoint全部闭合后追加final Post-run并同步
