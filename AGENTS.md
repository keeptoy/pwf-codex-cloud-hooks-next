...
- 智能体可以在活动任务授权范围内执行远端只读查询，准备精确命令和待推送的本地 commit，并在维护者完成
  远端动作后做只读 postflight；最终交接必须明确本地 commit、待维护者执行的远端动作和核验结果。
- 大白话：本地改完、测完就自动存成 commit；联网改 GitHub 的动作统一由维护者点按钮或执行命令。这样减少
  权限确认、网络重试和来回等待，通常也更省 token，但不得因此跳过测试、hash、资产或 postflight 校验。
