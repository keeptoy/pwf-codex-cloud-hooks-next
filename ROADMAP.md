...
授权并完成 F3C2 smart `S_ROLLBACK`：accepted v0.3.5、Fresh/real Resume、legacy profile、doctor、backup、零 residue 与
rollback evidence 已闭合；`S_RECOVER` 仍待独立 Cloud task执行，因此 F3C2 尚未整体完成。如果
...
|---|---|---|---|---|
| 4 | `0.4.0-*` | owned v3 state foundation；显式 smart/autonomous opt-in | F0 → F1A/F1B → F2A/F2B → F3A lifecycle foundation → F3B0～F3B4 Fresh/Resume/disarm/re-arm → F3C rollback；legacy 默认不变 | F1/F2A/F2B/F3A Source/Candidate Cloud PASS；F3B0～F3B4 complete；完整 F3B live lifecycle PASS；F3C Discovery conditional-go；F3C1 ref-aware Linux/no-live PASS；F3C2 `S_ROLLBACK` PASS、`S_RECOVER` pending |
...
已完成 `S_ROLLBACK`，只待独立 `S_RECOVER`；F3C3 live、F3C4 closure 与 ref mutation仍未授权。
