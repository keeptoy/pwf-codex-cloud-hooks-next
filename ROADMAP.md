...
| 当前开发列车 | `v0.4.0-dev` zero-hash development candidate；F0/F1A/F1B complete；F2A/F2B Source/Candidate Cloud PASS；F3A lifecycle foundation 已完成本地实施，Linux/Source-Candidate no-live 待验收；F3B/F3C 未授权 |
...
| 当前 programme 边界 | Product Phase 4；F1 foundation 与 F2A/F2B read-only consumer gates complete；F3 Discovery complete；F3A 本地 foundation complete，停在其 Linux/Source-Candidate no-live gate 之前 |
...
当前等待 Linux/Source-Candidate no-live 验收。F3B/F3C 仍未授权。如果 lifecycle 最终不可行，兜底仍是回退 smart-only
...
|---|---|---|---|---|
| 4 | `0.4.0-*` | owned v3 state foundation；显式 smart/autonomous opt-in | F0 → F1A/F1B → F2A/F2B → F3A lifecycle foundation → F3B Fresh/Resume/disarm/re-arm → F3C rollback；legacy 默认不变 | F1/F2A/F2B Source/Candidate Cloud PASS；F3A local complete、Linux/no-live pending；F3B/F3C 未授权 |
...
runbook 和 disposable-fixture 负向测试已经完成本地实施，停止线是 F3A Linux/Source-Candidate no-live gate 之前。该 gate
PASS 也不自动授权 F3B live Cloud；F3C 仍须等待 F3B PASS 与另一轮授权。
