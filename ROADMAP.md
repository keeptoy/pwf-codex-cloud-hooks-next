...
| 当前开发列车 | `v0.4.0-dev` zero-hash development candidate；F0/F1A/F1B complete；F2A implementation 本地完成，修正验收写权限后等待 Fresh no-live Cloud 重跑；上一轮 F1 zero-hash Source/Candidate/no-live Cloud PASS |
...
| 当前 programme 边界 | Product Phase 4 总体/F2A Discovery 与 F1 foundation complete；F2A implementation local complete，Cloud acceptance pending；F2B/F3 未授权 |
...
implementation 已获维护者授权并按该设计施工；完成本地闭合不等于 Linux/Cloud acceptance，也不授权 F2B/F3。
...
|---|---|---|---|---|
| 4 | `0.4.0-*` | owned v3 state foundation；显式 smart/autonomous opt-in | F0 development identity → F1A/F1B inactive foundation → F2A smart → F2B autonomous → F3 Fresh/Resume/rollback Cloud；legacy 默认不变 | F1 foundation/Cloud PASS；F2A implementation local complete、Fresh no-live Cloud pending；F2B/F3 未授权 |
...
Discovery，随后已获授权进入 F2A implementation；本地实现已经通过。首次 Linux/Source-Candidate no-live 执行暴露
Cloud 模型自动修复并创建 PR 的验收写权限缺口，只保留为诊断/功能证据；当前停止线是在协议修正后 Fresh 重跑并
闭合 exact HEAD/ZIP、Linux suite 与 B/D/E2 原始 evidence，不自动进入 F2B/F3。
