...
> `0.3.1 security-fix train` 正在进行；S1、S2 Linux/Cloud hard acceptance、S3-A 本地 immutable
> seal 与 S3-B publication 已完成；S3-C 已授权，本地 public-byte verification PASS，正在等待维护者
> 执行 final Cloud smoke。rollback/Latest promotion 尚未授权。
...
| 当前 programme gate | `0.3.1 security-fix train` S3-C public bytes PASS / final Cloud pending；Product Phase 4 未授权 |
| 当前 Release | `v0.3.1` 已发布、等待 S3-C；stable/accepted `v0.3.0` 仍为 rollback/Latest |
| 当前候选源码 | published `v0.3.1@9aa2148...`；双资产已上传且服务端 digest 匹配，尚未 S3-C accepted |
