...
维护者随后单独完成 F3C3 `A_ROLLBACK`：accepted v0.3.5 在 autonomous prepared残留状态上仍只产生 legacy context，
nonce/Plan-SHA/ledger renderer均未泄漏，activation未复活；rollback evidence已通过并停止在独立 `A_RECOVER` 授权前。如果
...
`A_ROLLBACK` accepted rollback并停止复核，`A_RECOVER`、F3C4 closure与 ref mutation仍未授权。
